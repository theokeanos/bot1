...
setInterval(() => {
  http.get(`https://sawn-supervizor.glitch.me/`);
}, 280000);
    ...
    if (reklam.some(word => message.content.toLowerCase().includes(word))) {
      if (!message.member.hasPermission("BAN_MEMBERS")) {
        ...
        if (uyarisayisi === 3) {
          message.delete();
          await kullanici.ban({
            reason: `Salvo Reklam-Engel Sistemi!`
          });
...
client.on("userUpdate", async (oldUser, newUser) => {
  ...
  const tag = '†'
  const sunucu = '802126801140580352'
  const kanal = '802126803921010710'
  const rol = '802126801169022992'
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'sa') {  //BUNU YAZINCA ALTTAKİ MESAJ GİDİCEK
    msg.reply('**Aleyküm Selam Dostum Hoşgeldin** <a:gul:802863196502228993>'); //YUKARIDAKİNİ YAZINCA BU CEVABI VERİCEK
  }
});

client.on('message', msg => {
  if (msg.content.toLowerCase() === 'sea') {  //BUNU YAZINCA ALTTAKİ MESAJ GİDİCEK
    msg.reply('**Aleyküm Selam Dostum Hoşgeldin** <a:gul:802863196502228993>'); //YUKARIDAKİNİ YAZINCA BU CEVABI VERİCEK
  }
});

client.on('message', msg => {
  if (msg.content.toLowerCase() === 'selamün aleyküm') {  //BUNU YAZINCA ALTTAKİ MESAJ GİDİCEK
    msg.reply('**Aleyküm Selam Dostum Hoşgeldin** <a:gul:802863196502228993>'); //YUKARIDAKİNİ YAZINCA BU CEVABI VERİCEK
  }
});

client.on('message', msg => {
  if (msg.content.toLowerCase() === 'selam') {  //BUNU YAZINCA ALTTAKİ MESAJ GİDİCEK
    msg.reply('**Aleyküm Selam Dostum Hoşgeldin** <a:gul:802863196502228993>'); //YUKARIDAKİNİ YAZINCA BU CEVABI VERİCEK
  }
});

client.on('message', msg => {
  if (msg.content.toLowerCase() === 'selamünaleyküm') {  //BUNU YAZINCA ALTTAKİ MESAJ GİDİCEK
    msg.reply('**Aleyküm Selam Dostum Hoşgeldin** <a:gul:802863196502228993>'); //YUKARIDAKİNİ YAZINCA BU CEVABI VERİCEK
  }
});
...
client.on("guildMemberAdd", async (member) => {
    ...
    if (ktarih < 604800000) {
    member.addRole("803043334540754984") //şüpheli rol id

    }else{

    member.addRole("") //güvenli rol id
